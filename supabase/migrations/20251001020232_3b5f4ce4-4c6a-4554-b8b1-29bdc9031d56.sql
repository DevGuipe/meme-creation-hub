-- ============================================
-- RESET TOTAL DO BANCO - VERSÃO FINAL
-- ============================================

-- 1. DROP TODAS AS POLICIES DE STORAGE (todas as possíveis variações)
DO $$ 
DECLARE
  r RECORD;
BEGIN
...
CREATE TYPE public.popcat_source AS ENUM (
  'save_meme', 'face_upload', 'publish', 'publish_group', 'reaction', 
  'reaction_thumbs', 'reaction_laugh', 'reaction_flex', 'reaction_popcat', 
  'reaction_moai', 'weekly_challenge', 'weekly_winner', 'tournament'
);
...
CREATE TABLE public.templates (
  key TEXT PRIMARY KEY, name TEXT NOT NULL, manifest_json JSONB NOT NULL,
  thumb_url TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TABLE public.assets (
  key TEXT PRIMARY KEY, type public.asset_type NOT NULL, url TEXT NOT NULL,
...
DECLARE short_id TEXT; attempts INTEGER := 0;
BEGIN
  LOOP
    ...
    END IF;
    attempts := attempts + 1;
    IF attempts >= 100 THEN RAISE EXCEPTION 'Could not generate unique short ID'; END IF;
  END LOOP;
END; $$;
...
BEGIN
  ...
  WITH user_scores AS (
    ...
    FROM public.users u LEFT JOIN public.popcat_events pe ON u.id = pe.user_id WHERE u.id = target_user_id GROUP BY u.id
  ),
  global_rank AS (
  ...
  SELECT us.uid, us.total, us.weekly, gr.rank, wr.rank FROM user_scores us
  LEFT JOIN global_rank gr ON us.uid = gr.id LEFT JOIN weekly_rank wr ON us.uid = wr.id;
END; $$;
...
BEGIN
  RETURN QUERY
  WITH user_scores AS (
    SELECT u.id as uid, u.telegram_id as utid, u.first_name as ufn,
  ...
  FROM user_scores us LEFT JOIN global_ranks gr ON us.uid = gr.uid
  LEFT JOIN weekly_ranks wr ON us.uid = wr.uid ORDER BY us.total DESC;
END; $$;
...
BEGIN
  ...
  WITH weekly_scores AS (
    ...
    FROM public.users u LEFT JOIN public.popcat_events pe ON u.id = pe.user_id
    WHERE pe.created_at >= date_trunc('week', CURRENT_DATE) AND pe.created_at < date_trunc('week', CURRENT_DATE) + INTERVAL '1 week'
    GROUP BY u.id, u.telegram_id, u.first_name HAVING COALESCE(SUM(pe.amount), 0) > 0
  )
...
BEGIN
  ...
  WITH period_scores AS (
    ...
    FROM public.users u LEFT JOIN public.popcat_events pe ON u.id = pe.user_id
    WHERE pe.created_at >= start_date::timestamptz AND pe.created_at <= end_date::timestamptz
    GROUP BY u.id, u.telegram_id, u.first_name HAVING COALESCE(SUM(pe.amount), 0) > 0
  )
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN QUERY SELECT m.id, m.id_short, m.owner_id, m.template_key, m.layers_payload, m.image_urls, m.created_at, m.deleted_at
  FROM public.memes m WHERE m.owner_id = user_uuid AND m.deleted_at IS NULL ORDER BY m.created_at DESC LIMIT 50;
END; $$;
...
CREATE POLICY "public_read_templates" ON public.templates FOR SELECT USING (true);
CREATE POLICY "service_templates" ON public.templates FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "public_read_assets" ON public.assets FOR SELECT USING (true);
CREATE POLICY "service_assets" ON public.assets FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "public_read_memes" ON public.memes FOR SELECT USING (deleted_at IS NULL);
CREATE POLICY "service_memes" ON public.memes FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "public_read_events" ON public.popcat_events FOR SELECT USING (true);
CREATE POLICY "service_events" ON public.popcat_events FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "public_read_leaderboard" ON public.leaderboard_snapshots FOR SELECT USING (true);
CREATE POLICY "service_leaderboard" ON public.leaderboard_snapshots FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "public_read_reactions" ON public.reactions FOR SELECT USING (true);
CREATE POLICY "service_reactions" ON public.reactions FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "public_read_reports" ON public.reports FOR SELECT USING (true);
CREATE POLICY "service_reports" ON public.reports FOR ALL USING (true) WITH CHECK (true);

-- STORAGE
INSERT INTO storage.buckets (id, name, public) VALUES ('memes', 'memes', true) ON CONFLICT (id) DO UPDATE SET public = true;

CREATE POLICY "storage_public_read" ON storage.objects FOR SELECT USING (bucket_id = 'memes');
CREATE POLICY "storage_service_write" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'memes');
CREATE POLICY "storage_service_update" ON storage.objects FOR UPDATE USING (bucket_id = 'memes');
CREATE POLICY "storage_service_delete" ON storage.objects FOR DELETE USING (bucket_id = 'memes');